...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<Map<String, Object>> signup(@Valid @RequestBody RegisterRequest request, HttpServletRequest httpRequest) {
        try {
            ...
            HttpSession session = httpRequest.getSession(true);
            session.setAttribute("pending_registration", request);
    ...
    public ResponseEntity<Map<String, Object>> verifyOtp(@RequestBody Map<String, String> request, HttpServletRequest httpRequest) {
        ...
        try {
            ...
            if (isValid) {
                ...
                RegisterRequest registerRequest = session != null
                        ? (RegisterRequest) session.getAttribute("pending_registration")
                        : null;

                if (registerRequest != null && registerRequest.getEmail().equals(email)) {
                    ...
                    log.info("✅ User registered and verified: {}", email);

                    // ✅ Null-safe response — no Map.of()
                    Map<String, Object> userData = new HashMap<>();
                    userData.put("id", userResponse.getId());
                    userData.put("username", userResponse.getUsername() != null ? userResponse.getUsername() : "");
                    userData.put("email", userResponse.getEmail() != null ? userResponse.getEmail() : "");
                    userData.put("fullName", userResponse.getFullName() != null ? userResponse.getFullName() : "");

                    Map<String, Object> response = new HashMap<>();
                    response.put("success", true);
                    response.put("message", "Email verified and registration complete!");
                    response.put("user", userData);
                    return ResponseEntity.ok(response);

                } else {
                    // ✅ OTP verified but no pending registration (e.g. resend OTP case)
                    Map<String, Object> response = new HashMap<>();
                    response.put("success", true);
